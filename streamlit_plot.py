...
class SOSReadonlyClient:
    ...
    def _get(self, endpoint: str, params: Optional[dict] = None, allowable_attempts: int = 3) -> dict:
        ...
        if endpoint_name == 'item':
            timeout_seconds = 30
            allowable_attempts = max(allowable_attempts, 4)
        elif endpoint_name == 'salesorder':
            timeout_seconds = 20
        else:
            timeout_seconds = 12
...
def render_sos_workspace():
    ...
    st.caption('Manual SOS checks use a longer timeout for heavy sales-order and item lookups so the Sales Order tab is less likely to fail.')
...
SOS_REDIRECT_URI="https://your-app.streamlit.app/""" , language='toml')
    ...
    with tab1:
        ...
        if cfind1.button('Find items', key='sos_find_single_items'):
            try:
                ...
                st.error(str(exc))
                st.caption("Tip: leave 'Explode subassemblies' unchecked first to get the base sales order result faster, then turn it on only when needed.")
    ...
    with tab3:
        ...
        explode = st.checkbox('Explode subassemblies', value=False, key='sos_so_explode')
...
def weekly_prod_state_path() -> Path:
    return WEEKLY_PROD_STATE_FILE


def weekly_gsheet_configured() -> bool:
    try:
        return bool(GSPREAD_AVAILABLE and st.secrets.get('google_sheet') and st.secrets.get('gcp_service_account'))
    except Exception:
        return False
...
def weekly_gsheet_backend_headers() -> list[str]:
    return [
        'so_number','product','priority','customer','product_type','notes','status','assigned_to','blocker',
        'last_updated_by','last_updated_at','qty_ordered','qty_shipped','qty_invoiced','qty_remaining',
        'date_shipped','tracking_number'
    ]


def weekly_gsheet_save_backend_board(board_df: pd.DataFrame) -> None:
    if not weekly_gsheet_configured():
        return
    board_df = weekly_production_normalize_df(board_df)
    ws = weekly_gsheet_get_worksheet()
    headers = weekly_gsheet_backend_headers()
    rows = []
    for _, row in board_df.iterrows():
        rows.append({
            ...
            'last_updated_by': str(row.get('Updated By', '') or ''),
            'last_updated_at': str(row.get('Last Updated At', '') or ''),
            'qty_ordered': int(pd.to_numeric(row.get('QTY Ordered', 0), errors='coerce') or 0),
            'qty_shipped': int(pd.to_numeric(row.get('QTY Shipped', 0), errors='coerce') or 0),
            'qty_invoiced': int(pd.to_numeric(row.get('QTY Invoiced', 0), errors='coerce') or 0),
            'qty_remaining': int(pd.to_numeric(row.get('QTY Remaining', 0), errors='coerce') or 0),
            'date_shipped': str(row.get('Date Shipped', '') or ''),
            'tracking_number': str(row.get('Tracking Number', '') or ''),
        })
    ws.clear()
...
def weekly_gsheet_load_backend_board() -> pd.DataFrame:
    if not weekly_gsheet_configured():
    ...
    required = {'so_number', 'product'}
    if not required.issubset(set(lower.keys())):
        return weekly_production_empty_df()
    def g(name):
        return lower.get(name, '')
    out = pd.DataFrame({
        'SO Number': df[g('so_number')].fillna('').astype(str) if g('so_number') else '',
        'Product': df[g('product')].fillna('').astype(str) if g('product') else '',
        'Priority': pd.to_numeric(df[g('priority')], errors='coerce').fillna(0).astype(int) if g('priority') else 0,
        'Customer': df[g('customer')].fillna('').astype(str) if g('customer') else '',
        'Product Type': df[g('product_type')].fillna('').astype(str) if g('product_type') else '',
        'Notes': df[g('notes')].fillna('').astype(str) if g('notes') else '',
        'Status': df[g('status')].fillna('').astype(str) if g('status') else '',
        'Assigned To': df[g('assigned_to')].fillna('').astype(str) if g('assigned_to') else '',
        'Blocker': df[g('blocker')].fillna('').astype(str) if g('blocker') else '',
        'Updated By': df[g('last_updated_by')].fillna('').astype(str) if g('last_updated_by') else '',
        'Last Updated At': df[g('last_updated_at')].fillna('').astype(str) if g('last_updated_at') else '',
        'QTY Ordered': pd.to_numeric(df[g('qty_ordered')], errors='coerce').fillna(0).astype(int) if g('qty_ordered') else 0,
        'QTY Shipped': pd.to_numeric(df[g('qty_shipped')], errors='coerce').fillna(0).astype(int) if g('qty_shipped') else 0,
        'QTY Invoiced': pd.to_numeric(df[g('qty_invoiced')], errors='coerce').fillna(0).astype(int) if g('qty_invoiced') else 0,
        'QTY Remaining': pd.to_numeric(df[g('qty_remaining')], errors='coerce').fillna(0).astype(int) if g('qty_remaining') else 0,
        'Date Shipped': df[g('date_shipped')].fillna('').astype(str) if g('date_shipped') else '',
        'Tracking Number': df[g('tracking_number')].fillna('').astype(str) if g('tracking_number') else '',
    })
    out = weekly_production_normalize_df(out)
    # Drop old closed/completed sales orders that are not shipped this week.
    if not out.empty:
        so_keep = []
        for so, block in out.groupby('SO Number', sort=False):
            remaining = pd.to_numeric(block['QTY Remaining'], errors='coerce').fillna(0)
            if (remaining > 0).any() or block.apply(weekly_is_row_shipped_this_week, axis=1).any():
                so_keep.append(so)
        out = out[out['SO Number'].isin(so_keep)].reset_index(drop=True)
    return out


def weekly_product_display(item_obj: Dict[str, Any], line_obj: Dict[str, Any]) -> str:
    part = _weekly_text(item_obj.get('name'), line_obj.get('itemName'), line_obj.get('name'))
    desc = _weekly_text(item_obj.get('fullname'), line_obj.get('description'), line_obj.get('itemDescription'))
    if desc and desc != part:
        return f"{part}, {desc}" if part else desc
    return part or desc
...
def weekly_prod_load_state_local() -> Dict[str, Any]:
    ...
    try:
        ...
        state = weekly_prod_state_default()
        state.update(raw)
        if not isinstance(state.get('priority_order'), list):
...
def weekly_prod_build_so_overrides(df: pd.DataFrame) -> Dict[str, Dict[str, Any]]:
    out: Dict[str, Dict[str, Any]] = {}
    if df is None or len(df) == 0:
        return out
    work = weekly_production_normalize_df(df)
    for so, block in work.groupby('SO Number', sort=False):
        if not so:
            continue
        first = block.iloc[0]
        out[str(so)] = {
            'Priority': int(pd.to_numeric(first.get('Priority', 0), errors='coerce') or 0),
            'Customer': str(first.get('Customer', '') or ''),
            'Status': str(first.get('Status', '') or ''),
            'Assigned To': str(first.get('Assigned To', '') or ''),
            'Blocker': str(first.get('Blocker', '') or ''),
            'Notes': str(first.get('Notes', '') or ''),
            'Updated By': str(first.get('Updated By', '') or ''),
            'Last Updated At': str(first.get('Last Updated At', '') or ''),
        }
    return out
...
def weekly_prod_apply_so_overrides(df: pd.DataFrame, overrides: Dict[str, Dict[str, Any]]) -> pd.DataFrame:
    ...
    for idx, row in out.iterrows():
        so = str(row.get('SO Number', '') or '').strip()
        ov = overrides.get(so) or {}
        if not ov:
            continue
        for col in ['Customer', 'Status', 'Assigned To', 'Blocker', 'Notes', 'Updated By', 'Last Updated At']:
            if col in ov and str(ov.get(col, '') or '') != '':
                out.at[idx, col] = ov.get(col, '')
        if ov.get('Priority') not in [None, '']:
            try:
                out.at[idx, 'Priority'] = int(float(ov.get('Priority') or 0))
            except Exception:
                pass
...
def weekly_build_so_editor_df(board_df: pd.DataFrame) -> pd.DataFrame:
    ...
    for so, block in board_df.groupby('SO Number', sort=False):
        ...
        rows.append({
            ...
            'Updated By': first.get('Updated By', ''),
...
def weekly_apply_so_editor_df(board_df: pd.DataFrame, so_editor_df: pd.DataFrame) -> pd.DataFrame:
    ...
    for _, row in work.iterrows():
        ...
        overrides[so] = {
            ...
            'Blocker': str(row.get('Blocker', '') or ''),
            'Notes': str(row.get('Notes', '') or ''),
            'Updated By': str(row.get('Updated By', '') or ''),
...
def weekly_parse_any_date(value: Any) -> Optional[pd.Timestamp]:
...
def weekly_dates_from_text(value: Any) -> List[pd.Timestamp]:
    text = str(value or '').strip()
    if not text:
        return []
    ...
    for part in parts:
        ts = weekly_parse_any_date(part)
        if ts is not None:
            found.append(ts)
    if not found:
        ts = weekly_parse_any_date(text)
        if ts is not None:
            found.append(ts)
    return found


def weekly_current_week_start(today: Optional[pd.Timestamp] = None) -> pd.Timestamp:
    base = pd.Timestamp(today or pd.Timestamp.today()).normalize()
    return base - pd.Timedelta(days=int(base.weekday()))


def weekly_is_date_in_current_week(value: Any) -> bool:
    week_start = weekly_current_week_start()
    week_end = week_start + pd.Timedelta(days=6)
    return any(week_start <= ts <= week_end for ts in weekly_dates_from_text(value))


def weekly_is_row_shipped_this_week(row: pd.Series) -> bool:
    try:
        remaining = int(pd.to_numeric(row.get('QTY Remaining', 0), errors='coerce'))
        shipped = int(pd.to_numeric(row.get('QTY Shipped', 0), errors='coerce'))
    except Exception:
        remaining, shipped = 0, 0
    if shipped <= 0:
        return False
    if not weekly_is_date_in_current_week(row.get('Date Shipped', '')):
        return False
    return remaining <= 0
...
def weekly_sort_so_numbers(sales_orders: List[str], priority_order: List[str]) -> List[str]:
    seen = set()
    ordered = []
    for so in priority_order or []:
        so = str(so or '').strip()
        if so and so in sales_orders and so not in seen:
            ordered.append(so)
            seen.add(so)
...
def weekly_extract_so_status(*objs: Dict[str, Any]) -> str:
    vals: List[str] = []
    for obj in objs:
        if not isinstance(obj, dict):
            continue
        for key in ['status', 'state', 'transactionStatus', 'orderStatus']:
            v = obj.get(key)
            if v:
                vals.append(str(v))
...
def weekly_status_is_closed(status_text: str) -> bool:
    t = str(status_text or '').lower()
    return any(x in t for x in ['closed', 'complete', 'completed', 'fully shipped', 'fulfilled'])


def weekly_status_is_openish(status_text: str) -> bool:
    t = str(status_text or '').lower()
    if not t:
        return True
    if weekly_status_is_closed(t):
        return False
...
def weekly_production_rows_from_sales_order(client: SOSReadonlyClient, so_number: str) -> pd.DataFrame:
    ...
    for ln in lines:
        item_obj = ln.get('item') or {}
        product = weekly_product_display(item_obj, ln)
        if not product:
            continue
    ...
    try:
        shipments = client.get_shipments_for_sales_order(so_number, maxresults=100)
    except Exception:
        shipments = []
    ...
    if not line_has_shipped_qty:
        for shipment in shipments:
            ...
            for sh_line in sh_lines:
                ...
                key = weekly_product_display(item_obj, sh_line)
                if not key:
...
def weekly_merge_live_board_with_state(live_df: pd.DataFrame, state: Dict[str, Any]) -> pd.DataFrame:
    ...
    out = weekly_prod_apply_so_overrides(live_df, overrides)
    ordered_sos = weekly_sort_so_numbers(
        list(dict.fromkeys(out['SO Number'].astype(str).tolist())),
        (state or {}).get('priority_order') or [],
    )
    return weekly_production_reset_priorities(out, ordered_sos)


def weekly_build_shipped_this_week_df(current_board: pd.DataFrame, open_sos: List[str]) -> pd.DataFrame:
    board = weekly_production_normalize_df(current_board)
    if board.empty:
        return weekly_production_empty_df()
    open_set = {str(x).strip() for x in open_sos}
    shipped_rows = board[board.apply(weekly_is_row_shipped_this_week, axis=1)].copy()
    if shipped_rows.empty:
        return weekly_production_empty_df()
    shipped_rows = shipped_rows[~shipped_rows['SO Number'].astype(str).isin(open_set)].copy()
    if shipped_rows.empty:
        return weekly_production_empty_df()
...
def weekly_prepare_display_df(board_df: pd.DataFrame) -> pd.DataFrame:
    ...
    for so, block in board_df.groupby('SO Number', sort=False):
        ...
        display_rows.append({
            ...
            'QTY Invoiced': int(pd.to_numeric(block['QTY Invoiced'], errors='coerce').fillna(0).sum()),
            'QTY Remaining': int(pd.to_numeric(block['QTY Remaining'], errors='coerce').fillna(0).sum()),
            'Status': first.get('Status', ''),
            'Assigned To': first.get('Assigned To', ''),
            'Blocker': first.get('Blocker', ''),
            'Notes': first.get('Notes', ''),
...
def weekly_render_display_table(board_df: pd.DataFrame, title: str, force_green: bool = False) -> None:
    ...
    try:
        styler = styler.hide(axis='columns', subset=['_row_kind', '_shipped_week', '_header_complete'])
        st.dataframe(styler, use_container_width=True, hide_index=True, height=min(680, 38 * (len(shown) + 1)))
    except Exception:
        shown = shown.drop(columns=['_row_kind', '_shipped_week', '_header_complete'], errors='ignore')
...
def weekly_resequence_priority_editor(previous_df: pd.DataFrame, edited_df: pd.DataFrame) -> pd.DataFrame:
    ...
    for i in range(min(len(prev), len(edited))):
        ...
        if prev_so != new_so:
            continue
        prev_pr = int(prev.iloc[i].get("Priority", 0) or 0)
        new_pr = int(edited.iloc[i].get("Priority", 0) or 0)
        if prev_pr != new_pr:
...
def weekly_apply_priority_change_inline(board_df: pd.DataFrame, edited_priority_rows: pd.DataFrame, save_changes: bool = True) -> pd.DataFrame:
    ...
    if updated_by and "Updated By" in resequenced_df.columns:
        resequenced_df["Updated By"] = updated_by
    if "Last Updated At" in resequenced_df.columns:
        resequenced_df["Last Updated At"] = now_text
...
def weekly_render_active_orders_inline(board_df: pd.DataFrame) -> pd.DataFrame:
    ...
    for _, row in display_df.iterrows():
        is_header = row.get("_row_kind") == "header"
        is_green = bool(row.get("_shipped_week")) or bool(row.get("_header_complete"))

        if is_header and bool(row.get("_header_complete")):
            bg = "#b7e1cd"
        elif is_header:
            bg = "#f4d36b"
        elif is_green:
        ...
        if is_header:
            ...
            priority_values[so] = int(max(1, original_priority))
            row_cols[0].markdown(
                f"<div style='background:{bg}; padding:0.45rem 0.5rem; border-radius:0.15rem; font-weight:700; min-height:2.4rem; white-space:nowrap; overflow:hidden; text-overflow:ellipsis;'>{int(max(1, original_priority))}</div>",
                unsafe_allow_html=True,
            )
            ...
            for cobj, value in zip(row_cols[1:], vals):
                ...
                cobj.markdown(
                    f"<div style='background:{bg}; padding:0.45rem 0.5rem; border-radius:0.15rem; font-weight:700; min-height:2.4rem; white-space:nowrap; overflow:hidden; text-overflow:ellipsis;' title='{safe_value}'>{safe_value}</div>",
                    unsafe_allow_html=True,
                )
        else:
            vals = [
...
def weekly_move_so_priority(board_df: pd.DataFrame, so_number: str, direction: str) -> pd.DataFrame:
    so_number = str(so_number or "").strip()
    board_df = weekly_production_normalize_df(board_df)
    editor_df = weekly_build_so_editor_df(board_df).copy().reset_index(drop=True)
    if editor_df.empty or not so_number:
        return board_df

    editor_df["SO Number"] = editor_df["SO Number"].astype(str).str.strip()
    order = editor_df["SO Number"].tolist()
    if so_number not in order:
        return board_df

    idx = order.index(so_number)
    if direction == "up" and idx > 0:
        ...
        order.pop(idx)
        order.insert(0, so_number)

    rank_map = {so: i + 1 for i, so in enumerate(order)}
    editor_df["_rank"] = editor_df["SO Number"].map(rank_map).fillna(999999).astype(int)
    editor_df = editor_df.sort_values("_rank", kind="stable").drop(columns=["_rank"]).reset_index(drop=True)
    editor_df["Priority"] = range(1, len(editor_df) + 1)

    now_text = pd.Timestamp.now().strftime("%Y-%m-%d %H:%M:%S")
    updated_by = st.session_state.get("weekly_updated_by", "").strip()
    if updated_by and "Updated By" in editor_df.columns:
        editor_df["Updated By"] = updated_by
    if "Last Updated At" in editor_df.columns:
        editor_df["Last Updated At"] = now_text

    return weekly_apply_so_editor_df(board_df, editor_df)
...
def render_weekly_production_workspace():
        ...
        if client is None:
            ...
            with refresh_feedback:
                with st.status('Refreshing Weekly Production board...', expanded=True) as status_box:
                    ...
                    try:
                        progress_ph.info('Connecting to SOS...')
                        ...
                        progress_ph.error(st.session_state['weekly_refresh_error'])
    ...
    with st.expander('Optional manual add / load', expanded=False):
        ...
        with left:
            ...
            a1, a2 = st.columns(2)
            if a1.button('Add one SO from SOS', key='weekly_add_from_sos', use_container_width=True):
                if client is None:
    ...
    priority_left, priority_right = st.columns([1.6, 1.4])
    with priority_left:
    ...
    with priority_right:
        b1, b2 = st.columns(2)
        if b1.button('Move selected SO up', key='weekly_move_up', use_container_width=True, disabled=not selected_so):
            board_df = weekly_production_reorder_so(board_df, selected_so, 'up')
            st.session_state['weekly_prod_df'] = board_df
            weekly_save_priority_state_from_board(board_df)
            st.rerun()
        if b2.button('Move selected SO down', key='weekly_move_down', use_container_width=True, disabled=not selected_so):
    ...
    if 'weekly_priority_preview_df' not in st.session_state:
        st.session_state['weekly_priority_preview_df'] = pd.DataFrame()

    so_priority_df = weekly_build_so_editor_df(board_df).copy()
    so_choices = so_priority_df['SO Number'].astype(str).tolist() if not so_priority_df.empty and 'SO Number' in so_priority_df.columns else []

    if so_choices:
        pr1, pr2, pr3, pr4 = st.columns([2.2, 1.0, 1.0, 1.0])
        selected_priority_so = pr1.selectbox('Sales Order priority', so_choices, key='weekly_priority_selected_so')
        move_up_clicked = pr2.button('Move selected SO up', key='weekly_move_up_btn', use_container_width=True)
        move_down_clicked = pr3.button('Move selected SO down', key='weekly_move_down_btn', use_container_width=True)
        move_top_clicked = pr4.button('Move selected SO to top', key='weekly_move_top_btn', use_container_width=True)

        working_board_df = st.session_state.get('weekly_priority_preview_df')
        if not isinstance(working_board_df, pd.DataFrame) or working_board_df.empty:
            working_board_df = board_df.copy()

        changed_order = False
        if move_up_clicked:
            working_board_df = weekly_move_so_priority(working_board_df, selected_priority_so, 'up')
            changed_order = True
        elif move_down_clicked:
            working_board_df = weekly_move_so_priority(working_board_df, selected_priority_so, 'down')
            changed_order = True
        elif move_top_clicked:
            working_board_df = weekly_move_so_priority(working_board_df, selected_priority_so, 'top')
            changed_order = True

        if changed_order:
            st.session_state['weekly_priority_preview_df'] = working_board_df
            board_df = working_board_df
            st.info('Priority order staged. Click Save priority order to commit.')

        if not st.session_state['weekly_priority_preview_df'].empty:
            if st.button('Save priority order', key='weekly_save_priority_order_btn', use_container_width=True):
                board_df = st.session_state['weekly_priority_preview_df'].copy()
                st.session_state['weekly_prod_df'] = board_df
                weekly_save_priority_state_from_board(board_df)
                st.session_state['weekly_priority_preview_df'] = pd.DataFrame()
    ...
    if 'weekly_inventory_result_so' not in st.session_state:
        st.session_state['weekly_inventory_result_so'] = ''
    ...
    def _render_inventory_panel(container):
        with container:
            ...
            with st.expander('Show detailed inventory result', expanded=False):
                st.dataframe(alloc_aware_df, use_container_width=True, hide_index=True, height=420)
                st.download_button(
    ...
    if view_mode == 'split':
        ...
        with left_board_col:
            board_df = weekly_render_active_orders_inline(board_df)
    ...
    with st.expander('Shipped this week', expanded=True):
        ...
        shipped_export = shipped_df.to_csv(index=False).encode('utf-8')
        st.download_button('Download shipped-this-week CSV', data=shipped_export, file_name='weekly_production_shipped_this_week.csv', mime='text/csv', use_container_width=True)

    with st.expander('How it works', expanded=False):
        st.write('• Refresh open sales orders pulls current open sales orders from SOS and rebuilds the active board.')
        st.write('• The app preserves sales-order priority and workflow fields using Google Sheets when configured, otherwise local JSON.')
